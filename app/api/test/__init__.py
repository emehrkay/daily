...
class BaseTestCase(AsyncHTTPTestCase):
    ...
    def send_request(self, url, callback, wait=True, **kwargs):
        """
        set wait to False if you're calling this method within the context of another reequest
            see the auth_tests logout for example
        """
...
class GraphTestMixin(object):
    ...
    def truncate_graph(self):
        script = "%s.V().map{it.get().remove()}" % (MAPPER.graph.gv)
